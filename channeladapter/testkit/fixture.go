...
func run(ctx context.Context, input io.Reader, output io.Writer, scripted bool, deliveryCount *int) error {
	...
	ready := protocol.Ready{ChannelKind: "fixture", Features: append([]protocol.Feature(nil), initPayload.Features...), Limits: initPayload.Limits}
	...
	if scripted {
		...
		inboundEnvelope = protocol.Envelope{ProtocolVersion: protocol.ProtocolVersion, ID: "fixture.inbound.1", Payload: protocol.InboundMessage{SourceID: "source.message.1", Route: protocol.Route{Handle: "route_1"}, Message: protocol.MessageRef{Handle: "message_1"}, Author: protocol.Author{Handle: "author_1", Label: "Operator"}, Text: "hello"}}
	...
	for {
		...
		case *protocol.EventAck:
			if scripted && frame.CorrelationID == inboundEnvelope.ID && !replayed {
				...
				status := protocol.Envelope{ProtocolVersion: protocol.ProtocolVersion, ID: "fixture.control.status.1", Payload: protocol.ControlRequest{SourceID: "source.status.1", Route: protocol.Route{Handle: "route_1"}, Message: protocol.MessageRef{Handle: "message_1"}, Action: protocol.ControlStatus}}
		...
		case *protocol.ControlResult:
			...
			if controlStage == 1 {
				...
				reset := protocol.Envelope{ProtocolVersion: protocol.ProtocolVersion, ID: "fixture.control.reset.1", Payload: protocol.ControlRequest{SourceID: "source.reset.1", Route: protocol.Route{Handle: "route_1"}, Message: protocol.MessageRef{Handle: "message_1"}, Action: protocol.ControlReset}}
		...
		case *protocol.InteractionCancel:
			answer := protocol.SemanticInteractionAnswer{SchemaVersion: 1, Action: protocol.AnswerCancel}
			if err := write(protocol.InteractionResult{InteractionID: payload.InteractionID, Answer: answer}, frame.ID); err != nil {
				return err
			}
