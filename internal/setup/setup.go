...
func filesFor(p *project.Project, executable string) (generatedSetup, error) {
	...
	if p.DiscordChannel != nil {
		instructions += "\n## Discord channel participation\n\nFor inputs explicitly marked as Discord messages, apply this participation policy before answering:\n\n" + strings.TrimSpace(string(p.DiscordChannel.Policy)) + "\n\nA Discord input marked `direct=true` is explicitly addressed to you; respond normally unless the user asks for silence. For other ambient guild messages, if no visible response is warranted, return exactly `HCTL_NO_REPLY` and nothing else. This control result applies only to Discord inputs.\n"
	}
