...
func filesForPolicyWithNativeMCP(p *project.Project, executable string, channelWritable bool, nativeMCP []integration.NativeMCPLaunchDescriptor) (generatedSetup, error) {
	...
	if p.GitHubConnection != nil {
		instructions += "\n## GitHub connection\n\n" + p.GitHubConnection.Description + "\n\nUse the native `github` MCP server's discovered tools for GitHub outcomes. This server and its effects are native and unmanaged: hctl does not filter, confirm, broker, or audit its calls.\n"
	}
	if p.FrictionNotes {
