...
func filesFor(p *project.Project, executable string) (generatedSetup, error) {
	...
	if p.DiscordChannel != nil {
		instructions += "\n## Discord channel participation\n\nFor inputs explicitly marked as Discord messages, apply this participation policy before answering:\n\n" + strings.TrimSpace(string(p.DiscordChannel.Policy)) + "\n\nA Discord input marked `direct=true` is explicitly addressed to you; respond normally unless the user asks for silence. For other ambient guild messages, if no visible response is warranted, return exactly `" + channelconfig.NoReplyResult + "` and nothing else. Channel sessions are enforced read-only. If the user's request genuinely requires changing the workspace and cannot be completed with read-only operations, return exactly `" + channelconfig.RequestWriteAccessResult + "` and nothing else; do not ask the native harness for permission or attempt to bypass the policy. These control results apply only to Discord inputs.\n"
	}
