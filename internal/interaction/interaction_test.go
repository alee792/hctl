...
func TestResolveUsesDistinctFormFieldKinds(t *testing.T) {
	request := readRequestFixture(t, "form")
	capabilities := fullCapabilities()
	capabilities.FormFieldKinds = []Kind{KindText}
	resolution, err := Resolve(request, capabilities)
	if err != nil || resolution.Mode != RenderTextFallback || resolution.Reason != ReasonFormFieldKind {
		t.Fatalf("mixed form resolution = %#v, %v", resolution, err)
	}
	request.Fields = []Field{request.Fields[2]}
	resolution, err = Resolve(request, capabilities)
	if err != nil || resolution.Mode != RenderNative {
		t.Fatalf("text-only form resolution = %#v, %v", resolution, err)
	}
}

func TestTextGrammarAndParser(t *testing.T) {
	tests := []struct {
		fixture string
		reply   string
		check   func(Answer) bool
	}{
		{"confirm", "yes", func(a Answer) bool { return *a.Fields[0].Confirmed }},
		{"choose_one", "2", func(a Answer) bool { return reflect.DeepEqual(a.Fields[0].OptionIDs, []string{"production"}) }},
		{"choose_many", "3, 1", func(a Answer) bool { return reflect.DeepEqual(a.Fields[0].OptionIDs, []string{"lint", "race"}) }},
		{"text", "whole\nreply", func(a Answer) bool { return *a.Fields[0].Text == "whole\nreply" }},
		{"date_time", "2026-08-07T09:30:00-07:00", func(a Answer) bool { return *a.Fields[0].DateTime == "2026-08-07T16:30:00Z" }},
		{"form", "environment: 2\nrelease_date: 2026-08-07\nnote: ship it", func(a Answer) bool { return len(a.Fields) == 3 && *a.Fields[2].Text == "ship it" }},
	}
	for _, test := range tests {
		t.Run(test.fixture, func(t *testing.T) {
			request := readRequestFixture(t, test.fixture)
			answer, err := ParseTextAnswer(request, test.reply)
			if err != nil || !test.check(answer) {
...
func TestTextParserIsStrictAndCancellationIsExact(t *testing.T) {
	confirm := readRequestFixture(t, "confirm")
	for _, invalid := range []string{"Yes", " yes", "yes\n", "1", "cancel\n"} {
		if _, err := ParseTextAnswer(confirm, invalid); err == nil {
			t.Fatalf("accepted %q", invalid)
		}
	}
	cancelled, err := ParseTextAnswer(confirm, "cancel")
	if err != nil || cancelled.Action != ActionCancel {
		t.Fatalf("cancel = %#v, %v", cancelled, err)
	}
	choice := readRequestFixture(t, "choose_many")
	for _, invalid := range []string{"01", "1,1", "0", "4", "1;2"} {
		if _, err := ParseTextAnswer(choice, invalid); err == nil {
			t.Fatalf("accepted choice %q", invalid)
		}
	}
	form := readRequestFixture(t, "form")
	for _, invalid := range []string{"unknown: value", "note: one\nnote: two", "environment 1"} {
		if _, err := ParseTextAnswer(form, invalid); err == nil {
			t.Fatalf("accepted form %q", invalid)
		}
	}
}

func TestTextParserRepresentsFreeformChoices(t *testing.T) {
	request := readRequestFixture(t, "choose_one")
	answer, err := ParseTextAnswer(request, "other=qa-east")
	if err != nil || answer.Fields[0].Freeform == nil || *answer.Fields[0].Freeform != "qa-east" {
		t.Fatalf("choose one freeform = %#v, %v", answer, err)
	}
	request.Kind, request.Field.Kind = KindChooseMany, KindChooseMany
	request.Field.MinSelections, request.Field.MaxSelections = 1, 3
	answer, err = ParseTextAnswer(request, "1;other=qa-east")
	if err != nil || !reflect.DeepEqual(answer.Fields[0].OptionIDs, []string{"staging"}) || answer.Fields[0].Freeform == nil || *answer.Fields[0].Freeform != "qa-east" {
		t.Fatalf("choose many mixed = %#v, %v", answer, err)
	}
	answer, err = ParseTextAnswer(request, "other=qa-west")
	if err != nil || answer.Fields[0].Freeform == nil || *answer.Fields[0].Freeform != "qa-west" {
		t.Fatalf("choose many freeform = %#v, %v", answer, err)
	}
	request.Kind, request.Field.Kind = KindChooseOne, KindChooseOne
	request.Field.MinSelections, request.Field.MaxSelections, request.Field.MinLength = 1, 1, 0
	answer, err = ParseTextAnswer(request, "other=")
	if err != nil || answer.Fields[0].Freeform == nil || *answer.Fields[0].Freeform != "" {
		t.Fatalf("choose one empty freeform = %#v, %v", answer, err)
	}
	request.Kind, request.Field.Kind = KindChooseMany, KindChooseMany
	request.Field.MinSelections, request.Field.MaxSelections = 1, 3
	answer, err = ParseTextAnswer(request, "other=")
	if err != nil || answer.Fields[0].Freeform == nil || *answer.Fields[0].Freeform != "" {
		t.Fatalf("choose many empty freeform = %#v, %v", answer, err)
	}
}
...
func fullCapabilities() Capabilities {
	return Capabilities{
		...
		FormFieldKinds:          []Kind{KindConfirm, KindChooseOne, KindChooseMany, KindText, KindDateTime},
		MaxRequestBytes:         MaxRequestBytes,
		MaxPromptBytes:          MaxPromptBytes,
		MaxFields:               MaxFields,
