...
| Schedule | A root-agent Markdown task under `schedules/` whose path is its name, whose frontmatter contains a cron cadence, and whose body is the prompt. Apply validates it without starting a clock; one-shot dispatch opens a fresh native-harness session, bounds its turn independently, and discards model text. |
