...
func filesForPolicyWithNativeMCP(p *project.Project, executable string, channelWritable bool, nativeMCP []integration.NativeMCPLaunchDescriptor) (generatedSetup, error) {
	...
	if p.GitHubConnection != nil {
		instructions += "\n## GitHub connection\n\n" + p.GitHubConnection.Description + "\n\nUse the native `github` MCP server's discovered tools by intended outcome; its live catalog and schemas are authoritative. This server and its effects are native and unmanaged: hctl does not filter, confirm, broker, or audit its calls, and workspace read-only policy does not constrain GitHub effects allowed by the ambient PAT. Apply does not grant native project, server, or tool trust; an operator must establish it deliberately before unattended launch. Native Git and `gh` require separately operator-configured authentication.\n"
	}
	if p.FrictionNotes {
	...
	if p.Harness == "claude" {
		...
		for _, server := range nativeMCP {
			mcpServers[server.ServerName] = claudeNativeMCPServer(server)
		}
...
func ValidateNativeMCP(p *project.Project, servers []integration.NativeMCPLaunchDescriptor) error {
	if p.GitHubConnection == nil {
		if len(servers) != 0 {
			return errors.New("native MCP configuration was supplied without an authored connection")
		}
		return nil
	}
	if len(servers) != 1 || servers[0].ServerName != "github" {
		return errors.New("GitHub connection requires the exact installed github native MCP capability")
	}
	server := servers[0]
	if server.Target.Name != p.Harness || server.Target.Trust != integration.TrustNativeProject || server.Target.Startup != integration.StartupOptional || !filepath.IsAbs(server.Command) || !filepath.IsAbs(server.WorkingDirectory) {
		return errors.New("GitHub native MCP launch metadata does not match the supported harness contract")
	}
	for _, plugin := range p.PluginMCPServers {
		if plugin.Name == server.ServerName {
			return fmt.Errorf("native MCP server %q collides with an authored plugin server", server.ServerName)
		}
	}
	return nil
}
...
func codexMCPConfig(p *project.Project, executable string, mcpArgs []string, nativeMCP []integration.NativeMCPLaunchDescriptor) ([]byte, error) {
	...
	for _, server := range nativeMCP {
		config.WriteString("\n[mcp_servers." + tomlQuote(server.ServerName) + "]\n")
