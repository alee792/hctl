...
| Native deferred-tool continuation | A parked interaction continuation that later resumes the same logical tool call through a harness-native continuation identity. The Claude adapter for this mode is deferred; the durable lifecycle does not imply that it is live. |
