# ADR 0037: Add the revision leg to the north star's measure
...
The north star's measure gains a third leg. It now reads: empty directory
to a working agent inside the author's harness in five minutes; the same
folder runs headless, scheduled, or staged without edits; and a revision
...
2. Published research on harness engineering for self-improvement (Weng,
   ...
   provides — file-represented editable components, bounded editable
   surfaces, attribution of gains to exact configurations, and permission
   control outside the loop — as the missing infrastructure of
   self-improving agent systems.
3. Harness-updating capability has been observed to be roughly flat
   across model sizes, so loop-driven authorship will be exercised by
   cheap models everywhere; well-formedness validation, not drafting
   skill, becomes the binding constraint.

## Consequences

Every slice can be asked which leg of the measure it improves, and a
