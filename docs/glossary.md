...
| Managed channel input tool | The capability-gated `channel.request_input` MCP tool available only to a proven channel-root session with a harness continuation strategy and compatible responder. It accepts only a transport-neutral interactive request; trusted runtime boundaries inject ownership and correlation before the dispatcher commits it. |
...
| Continuation turn | A parked interaction continuation that later opens another turn in the same native session with the normalized answer and request context. The Codex adapter for this mode is deferred; it is not the same tool call and is not yet live. |
